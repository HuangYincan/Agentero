...
标题栏 Layout 菜单中的窗口布局预设改变当前论文的 Notes 分屏、外层 Agent panel 宽度和左侧 Vault sidebar：Agent 模式为 PDF / Agent `1:1` 并展开左侧栏，笔记模式为 PDF / Notes / Agent `1:1:1` 并展开左侧栏，阅读模式关闭 Notes、Agent 与左侧栏，只保留 PDF 阅读界面。其它 PDF tab 保持打开。
